...
def leviatanscans_server():
    from komikku.servers.leviatanscans import Leviatanscans
...
def leviatanscans_es_server():
    from komikku.servers.leviatanscans import Leviatanscans_es
...
def test_leviatanscans(leviatanscans_server):
    ...
    try:
        response = leviatanscans_server.get_latest_updates()
    ...
    try:
        ...
        response = leviatanscans_server.search(response[0]['name'])
    ...
    try:
        response = leviatanscans_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
...
def test_leviatanscans_es(leviatanscans_es_server):
    ...
    print('Get latest updates')
    try:
        response = leviatanscans_es_server.get_latest_updates()
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    try:
        # Use first result of get_most_populars
        response = leviatanscans_es_server.search(response[0]['name'])
        slug = response[0]['slug']
    except Exception as e:
        slug = None
        log_error_traceback(e)
    ...
    print('Get page image')
    try:
        response = leviatanscans_es_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
