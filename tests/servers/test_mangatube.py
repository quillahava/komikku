...
@pytest.fixture
def mangatube_server():
    from komikku.servers.mangatube import Mangatube

    return Mangatube()


@test_steps('get_latest_updates', 'get_most_populars', 'search', 'get_manga_data', 'get_chapter_data', 'get_page_image')
def test_mangatube(mangatube_server):
    # Get latest updates
    print('Get latest updates')
    try:
        response = mangatube_server.get_latest_updates()
    ...
    try:
        response = mangatube_server.get_most_populars()
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    try:
        response = mangatube_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = mangatube_server.get_manga_chapter_data(slug, None, chapter_slug, None)
