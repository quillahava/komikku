...
@pytest.fixture
def dankefurslesen_server():
    from komikku.servers.dankefurslesen import Dankefurslesen
...
def test_dankefurslesen(dankefurslesen_server):
    ...
    try:
        ...
        response = dankefurslesen_server.search(response[1]['name'])
    ...
    try:
        response = dankefurslesen_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = dankefurslesen_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
