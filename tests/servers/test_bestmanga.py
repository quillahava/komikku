...
@pytest.fixture
def bestmanga_server():
    from komikku.servers.bestmanga import Bestmanga

    return Bestmanga()


@test_steps('get_latest_updates', 'get_most_populars', 'search', 'get_manga_data', 'get_chapter_data', 'get_page_image')
def test_bestmanga(bestmanga_server):
    # Get latest updates
    print('Get latest updates')
    try:
        response = bestmanga_server.get_latest_updates()
    ...
    try:
        response = bestmanga_server.get_most_populars()
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    try:
        response = bestmanga_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = bestmanga_server.get_manga_chapter_page_image(slug, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
