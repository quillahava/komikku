...
@pytest.fixture
def flamescans_server():
    from komikku.servers.flamescans import Flamescans

    return Flamescans()


@test_steps('get_latest_updates', 'get_most_populars', 'search', 'get_manga_data', 'get_chapter_data', 'get_page_image')
def test_flamescans(flamescans_server):
    # Get latest updates
    print('Get latest updates')
    try:
        response = flamescans_server.get_latest_updates('')
    ...
    try:
        response = flamescans_server.get_most_populars('')
    except Exception as e:
        response = None
        log_error_traceback(e)
    ...
    try:
        ...
        response = flamescans_server.search(response[0]['name'], '')
    ...
    try:
        response = flamescans_server.get_manga_data(dict(slug=slug))
