...
@pytest.fixture
def bilibili_server():
    from komikku.servers.bilibili import Bilibili
...
def test_bilibili(bilibili_server):
    ...
    try:
        response = bilibili_server.get_manga_data(dict(slug=slug))
    ...
    try:
        response = bilibili_server.get_manga_chapter_data(None, None, chapter_slug, None)
    ...
    try:
        response = bilibili_server.get_manga_chapter_page_image(None, None, chapter_slug, page)
    except Exception as e:
        response = None
        log_error_traceback(e)
